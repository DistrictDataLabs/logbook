...
fixtures = {
    ...
    'role': {
        'name': 'Student',
        'details': 'Someone who takes a DDL course',
    }
}
...
class MembershipModelTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(**fixtures['user'])
        self.role = Role.objects.create(**fixtures['role'])

    def test_membership_deactivate(self):
        """
        On marking inactive, the membership should deactivate.
        """
        membership = Membership.objects.create(profile=self.user.profile, role=self.role)
        self.assertTrue(membership.active)
        self.assertIsNone(membership.deactivated)

        membership.active = False
        membership.save()

        self.assertIsNotNone(membership.deactivated)
